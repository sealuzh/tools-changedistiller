...
public class JavaASTHelper implements ASTHelper<JavaStructureNode> {
	...
	@Inject
	JavaASTHelper(@Assisted File file, @Assisted String javaVersion,
		...
		fCompilation = JavaCompilationUtils.compile(file,
				convertStringToJdtConstant(javaVersion));
	...
	private long convertStringToJdtConstant(String javaVersion) {
		long versionNumber;
	...
	private Node createDeclarationTree(ASTNode astNode, Node root) {
		...
		if (astNode instanceof TypeDeclaration) {
			...
			((AbstractMethodDeclaration) astNode).traverse(
					fDeclarationConverter, (ClassScope) null);
	...
	public StructureEntityVersion createMethodInClassHistory(
		...
		StructureEntityVersion method = createStructureEntityVersion(node,
				versionNum);
		if (classHistory.getMethodHistories().containsKey(
	...
	public StructureEntityVersion createFieldInClassHistory(
		...
		StructureEntityVersion attribute = createStructureEntityVersion(node,
				versionNum);
		if (classHistory.getAttributeHistories().containsKey(
				attribute.getUniqueName())) {
			ah = classHistory.getAttributeHistories().get(
					attribute.getUniqueName());
	...
	public StructureEntityVersion createInnerClassInClassHistory(
		...
		StructureEntityVersion clazz = createStructureEntityVersion(node,
				versionNum);
		if (classHistory.getInnerClassHistories().containsKey(
				clazz.getUniqueName())) {
			ch = classHistory.getInnerClassHistories().get(
					clazz.getUniqueName());
			ch.addVersion(clazz);
		} else {
			ch = new ClassHistory(clazz);
			classHistory.getInnerClassHistories()
					.put(clazz.getUniqueName(), ch);
		}
		return clazz;
